...
public class IntegrationTestChatService {
	...
	{
		int clientCount = 50;// 2000; // feel free to push limits harder - but for CI we relax.
	...
	private void createTestUsers(int clientCount) throws IOException, ClientProtocolException {
		...
		String url = EngineUrl.process() + "/ConnectivityDemosTest/" + createUsers + 
	...
	private Integer countConnectedClients() {
		Integer cliCount = silentClient().target(chatResource+"/count").request().get().readEntity(Integer.class);
