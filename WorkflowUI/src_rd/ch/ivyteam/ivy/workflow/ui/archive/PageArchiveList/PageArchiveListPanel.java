package ch.ivyteam.ivy.workflow.ui.archive.PageArchiveList;
...
public class PageArchiveListPanel extends RichDialogGridBagPanel 
...
private RLabel getTaskFilterLabel() {
	if (taskFilterLabel == null) {
		...
		taskFilterLabel.setText("ivy.cms.co(\"/ch/ivyteam/ivy/workflow/ui/task/plainStrings/taskName\")");
...
private RTable getPageArchiveTable() {
	if (pageArchiveTable == null) {
		...
		pageArchiveTable.setModelConfiguration("{/showTableheader true /autoTableheader false /showtooltip false /showIcons false /version \"3.0\"/emptyTableText \"\"/columns {{/result \"result=entry.getTask().getName() + \\\" (\\\" + entry.getTask().getIdentifier() + \\\")\\\"\"/version \"3.0\"/tooltip \"\"/icon \"\"/header \"ivy.cms.co(\\\"/ch/ivyteam/ivy/workflow/ui/task/plainStrings/taskName\\\")\"/field \"\"/editable \"\"/condition \"!in.hideTaskFilter\"/cellWidget \"\"}{/result \"result=(entry.getSubmitTimestamp() is initialized ? new DateTime(entry.getSubmitTimestamp()).format(\\\"medium\\\"): \\\"n/a\\\")\"/version \"3.0\"/tooltip \"\"/icon \"\"/header \"ivy.cms.co(\\\"/ch/ivyteam/ivy/workflow/ui/archive/plainStrings/creationDate\\\")\"/field \"\"/editable \"\"/condition \"\"/cellWidget \"\"}{/result \"result=entry.getName()\"/version \"3.0\"/tooltip \"\"/icon \"\"/header \"ivy.cms.co(\\\"/ch/ivyteam/ivy/workflow/ui/common/plainStrings/name\\\")\"/field \"\"/editable \"\"/condition \"\"/cellWidget \"\"}{/result \"result=entry.getUserName()\"/version \"3.0\"/tooltip \"\"/icon \"\"/header \"ivy.cms.co(\\\"/ch/ivyteam/ivy/workflow/ui/common/plainStrings/user\\\")\"/field \"\"/editable \"\"/condition \"\"/cellWidget \"\"}}}");
		pageArchiveTable.setAutoResizeMode(ULCTable.AUTO_RESIZE_NEXT_COLUMN);
