[Ivy]
[>Created: Mon Nov 09 13:31:10 CET 2015]
119C23018068C697 3.18 #module
...
' #txt
Cs0 f1 embeddedRdInitializations '{/fileManagerRDC {/fieldName "fileManagerRDC"/startMethod "startWithServerPathAndUseIvySystemDB(String)"/parameterMapping ""/initScript "import ch.ivyteam.ivy.addons.restricted.resources.ResourceServicesHelper;\n\n// param.serverPath = \"\";\nparam.serverPath = ResourceServicesHelper.readCaseDocumentsPath(in.fCase.getId());\n"/userContext * }/caseNoteDisplayListRDC {/fieldName "caseNoteDisplayListRDC"/startMethod "start(ch.ivyteam.ivy.workflow.ICase,Boolean)"/parameterMapping ""/initScript "param.aCase = callParam.aCase;\nparam.aLoadDataAtStart = false;"/userContext * }/eventLogDisplayListRDC {/fieldName "eventLogDisplayListRDC"/startMethod "start()"/parameterMapping ""/initScript ""/userContext * }/workflowEventsDisplayListRDC {/fieldName "workflowEventsDisplayListRDC"/startMethod "start()"/parameterMapping ""/initScript ""/userContext * }/taskDisplayListRDC {/fieldName "taskDisplayListRDC"/startMethod "start(Boolean,ch.ivyteam.ivy.richdialog.widgets.displays.RCloseableTabbedDisplay,ch.ivyteam.ivy.workflow.IPropertyFilter,Number,Boolean)"/parameterMapping ""/initScript "param.aRunningTaskMode = true;\nparam.aTaskDisplayListParentDisplay = callParam.aParentDisplay;\nparam.aTaskDisplayMode = -1;\nparam.aHasWfAdministratorPermissions = ivy.session.hasPermission(ivy.request.getApplication().getSecurityDescriptor(), ch.ivyteam.ivy.security.IPermission.TASK_READ_ALL);"/userContext * }/formArchiveDisplayListRDC {/fieldName "formArchiveDisplayListRDC"/startMethod "start()"/parameterMapping ""/initScript ""/userContext * }/caseDetailsDisplayRDC {/fieldName "caseDetailsDisplayRDC"/startMethod "start(ch.ivyteam.ivy.workflow.ICase)"/parameterMapping ""/initScript "param.aCase = in.fCase;"/userContext * }}' #txt
