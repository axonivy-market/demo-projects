...
%>
	...
	<tr style="height:25px" >
		...
		<% } else { %> 
				<td class="labelTd"><a class='disabled' disabled='disabled'   href="#" title="<%=ivy.cms.co("/tooltips/resetTask")%>"><%=ivy.cms.co("/images/reset")%> <%=ivy.cms.co("/labels/taskReset")%></a></td>
		<% } 
		if (state==TaskState.DONE || state==TaskState.DESTROYED || state==TaskState.RESUMED || state==TaskState.FAILED || !hasDelegatePermission)	
		{ %>
				<td class="labelTd"><a  class='disabled' disabled='disabled'  href="#" title="<%=ivy.cms.co("/tooltips/delegateTask")%>"><%=ivy.cms.co("/images/delegate")%> <%=ivy.cms.co("/labels/taskDelegate")%></a></td>
		<% } else { %>
		...
		{ %>
				<td class="labelTd"><a  class='disabled' disabled='disabled'  href="#" title="<%=ivy.cms.co("/tooltips/changeExpiry")%>"><%=ivy.cms.co("/images/expiry")%> <%=ivy.cms.co("/labels/changeExpiry")%></a></td>
		<% } else { %>
	...
	<tr>
		...
		<td title="CaseId [<%=task.getCase().getId()%>]"><%=task.getCase().getName()%> Case[<%=task.getCase().getId()%>]</td>
	</tr>	
	<tr>
