...
Ps0 @RichDialogBroadcastStart f38 '' #zField
Ps0 @RichDialogEnd f39 '' #zField
...
if(in.selectedTask != null)
{
		...
		Date d = new Date();
		panel.CardDisplay.setDisplayId("execution_display"+d.toNumber());

		displayConfig = DisplayConfigurationFactory.createForThisWindow("execution_display"+d.toNumber(), panel);
		taskStartConfig = new TaskStartConfiguration(in.selectedTask, displayConfig);
		ivy.rd.startTask(taskStartConfig);
		panel.captionLabel.text = in.selectedTask.getName();
		panel.notesButton.text = ivy.cms.co("/ch/ivyteam/ivy/basicwfui/labels/task/notes")
		+" ("
...
{
	panel.resetButton1.visible = true;	
}
else
{
		panel.resetButton1.visible = false;
}
...
Ps0 f29 richDialogId ui.task.TaskInfo #txt
Ps0 f29 startMethod start(ch.ivyteam.ivy.workflow.ITask) #txt
Ps0 f29 type ui.task.TaskExecution.TaskExecutionData #txt
Ps0 f29 requestActionDecl '<ch.ivyteam.ivy.workflow.ITask task> param;' #txt
Ps0 f29 requestMappingAction 'param.task=in.selectedTask;
' #txt
...
Ps0 f38 guid 1270454D7B5B7AAC #txt
Ps0 f38 type ui.task.TaskExecution.TaskExecutionData #txt
Ps0 f38 broadcast wfUserChanged #txt
Ps0 f38 actionDecl 'ui.task.TaskExecution.TaskExecutionData out;
' #txt
...
' #txt
Ps0 f38 334 486 20 20 13 0 #rect
Ps0 f38 @|RichDialogBroadcastStartIcon #fIcon
Ps0 f39 type ui.task.TaskExecution.TaskExecutionData #txt
Ps0 f39 guid 1270454EA9FE3ACB #txt
Ps0 f39 334 630 20 20 13 0 #rect
Ps0 f39 @|RichDialogEndIcon #fIcon
Ps0 f40 expr out #txt
Ps0 f40 344 506 344 630 #arcP
