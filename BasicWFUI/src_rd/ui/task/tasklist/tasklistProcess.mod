[Ivy]
[>Created: Wed Feb 24 17:27:09 CET 2010]
...
Ps0 @RichDialogProcessStep f42 '' #zField
Ps0 @RichDialogProcessStart f43 '' #zField
Ps0 @PushWFArc f45 '' #zField
Ps0 @RichDialogProcessEnd f44 '' #zField
...
panel.taskNotesButton.text = ivy.cms.co("/ch/ivyteam/ivy/basicwfui/labels/task/notes");


if(!ivy.session.isSessionUserUnknown()
&& ivy.session.getSessionUser().getSecurityContext().hasPermission(ivy.wf.getApplication().getSecurityDescriptor(),ch.ivyteam.ivy.security.IPermission.TASK_RESET))
{
	panel.resetButton.visible = true;	
}
else
{
...
if(!ivy.session.isSessionUserUnknown()
&& ivy.session.getSessionUser().getSecurityContext().hasPermission(ivy.wf.getApplication().getSecurityDescriptor(),ch.ivyteam.ivy.security.IPermission.TASK_PARK_OWN_WORKING_TASK))
{
	panel.parkButton.visible = true;	
}
else
{
		panel.parkButton.visible = false;
}' #txt
...
{
	out.selectedTask= panel.Table.selectedListEntry as ch.ivyteam.ivy.workflow.ITask;
	panel.taskNotesButton.text = ivy.cms.co("/ch/ivyteam/ivy/basicwfui/labels/task/notes")
		+" ("
...
if(out.selectedTask !=null && (out.selectedTask.getState() == TaskState.SUSPENDED || out.selectedTask.getState()== TaskState.PARKED))
{	
		panel.resumeMenuItem.enabled =true;
}
else
{
	panel.resumeMenuItem.enabled =false;
}' #txt
...
' #txt
Ps0 f10 windowConfiguration '{/title "<%=ivy.cms.co(\"/ch/ivyteam/ivy/basicwfui/labels/info/taskDetails\")%>"/width 860 /height 680 /centered true /resizable true /maximized false /close_after_last_rd true }' #txt
...
Ps0 f33 windowConfiguration '{/title "<%=ivy.cms.co(\"/ch/ivyteam/ivy/basicwfui/labels/notes/annotations\")%>"/width 700 /height 500 /centered true /resizable true /maximized false /close_after_last_rd true }' #txt
...
' #txt
Ps0 f42 actionDecl 'ui.task.tasklist.tasklistData out;
' #txt
Ps0 f42 actionTable 'out=in;
' #txt
Ps0 f42 actionCode ' if (in.selectedTask.getState() == ch.ivyteam.ivy.workflow.TaskState.CREATED || 
	          in.selectedTask.getState() == ch.ivyteam.ivy.workflow.TaskState.RESUMED)
{
								// park the task
...
' #txt
Ps0 f42 type ui.task.tasklist.tasklistData #txt
Ps0 f42 342 572 36 24 23 -9 #rect
Ps0 f42 @|RichDialogProcessStepIcon #fIcon
Ps0 f43 guid 126F5F611785FA99 #txt
Ps0 f43 type ui.task.tasklist.tasklistData #txt
Ps0 f43 actionDecl 'ui.task.tasklist.tasklistData out;
' #txt
...
' #txt
Ps0 f43 350 502 20 20 13 0 #rect
Ps0 f43 @|RichDialogProcessStartIcon #fIcon
Ps0 f45 expr out #txt
Ps0 f45 360 522 360 572 #arcP
Ps0 f44 type ui.task.tasklist.tasklistData #txt
Ps0 f44 350 638 20 20 13 0 #rect
Ps0 f44 @|RichDialogProcessEndIcon #fIcon
Ps0 f46 expr out #txt
Ps0 f46 360 596 360 638 #arcP
>Proto Ps0 .type ui.task.tasklist.tasklistData #txt
>Proto Ps0 .processKind RICH_DIALOG #txt
>Proto Ps0 .rdData2UIAction 'panel.parkButton.enabled=in.selectedTask!=null && in.selectedTask.getState() == ch.ivyteam.ivy.workflow.TaskState.RESUMED;
panel.resetButton.enabled=in.selectedTask!=null && in.selectedTask.getState() == ch.ivyteam.ivy.workflow.TaskState.RESUMED;
panel.resumeButton.enabled=in.selectedTask!=null && in.selectedTask.getState() != ch.ivyteam.ivy.workflow.TaskState.RESUMED;
...
Ps0 f43 mainOut f45 tail #connect
Ps0 f45 head f42 mainIn #connect
Ps0 f42 mainOut f46 tail #connect
Ps0 f46 head f44 mainIn #connect
